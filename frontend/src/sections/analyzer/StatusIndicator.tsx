/**
 * Status dot + last-call text rendered on the analyzer canvas card.
 *
 * Reuses useAnalyzerLogStore (already polls /api/analyzer/log every 3s) — no
 * extra fetch. The dot reflects orchestrator state + queue backlog + the
 * latest call's verdict, so analyzer health is visible at a glance without
 * opening the Calls tab. Mirrors NewsSourceStatusIndicator.
 */
import { formatRelativeAgo, formatUTC } from '../news_source/time'
import { useAnalyzerLogStore } from './logStore'
...
export function AnalyzerStatusIndicator() {
  const data = useAnalyzerLogStore((s) => s.data)
  const fetchStatus = useAnalyzerLogStore((s) => s.status)

  // Priority: unreachable / stopped → error → busy → running. A historical
  // error counter does NOT keep the dot red — only the *latest* call's
  // verdict does, so a one-off failure clears on the next healthy call.
  ...
  if (fetchStatus === 'error') {
    ...
    if (latest?.verdict === 'error') {
      ...
      dotState = 'busy'
      text = `Analyzing · ${data.queue_depth} queued`
